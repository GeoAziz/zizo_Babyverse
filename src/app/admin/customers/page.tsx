...
export default function CustomerManagementPage() {
  ...
  useEffect(() => {
    // Create mock customers from orders with error handling
    try {
      setIsLoading(true);
      ...
      mockOrders.forEach(order => {
        if (!customerData[order.userId]) {
          customerData[order.userId] = {
            id: order.userId,
            name: `Customer ${order.userId.split('_')[1]}`,
                    ...
                    <TableBody>
                      {filteredCustomers.map((customer) => (
                        <TableRow key={customer.id}>
                          ...
                          <TableCell>{customer.lastOrderDate ? format(new Date(customer.lastOrderDate), 'MMM d, yyyy') : 'No orders'}</TableCell>
