...
interface MockCustomer extends User {
  ...
  useEffect(() => {
    ...
    mockOrders.forEach(order => {
      if (!customerData[order.userId]) {
        ...
        if (new Date(order.orderDate) > new Date(customerData[order.userId].lastOrderDate)) {
          customerData[order.userId].lastOrderDate = order.orderDate;
        }
      }
    });
