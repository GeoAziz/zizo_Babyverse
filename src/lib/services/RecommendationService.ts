...
export class RecommendationService {
  ...
  private static async generateRecommendations(
    ...
    const products = await prisma.product.findMany({
      where: {
        stock: { gt: 0 },
        // Add age-based filtering if baby profile exists
        ...(context.babyProfile && {
          OR: [
            { ageGroup: 'All Ages' },
            { ageGroup: { contains: `${Math.floor(context.babyProfile.ageInMonths / 12)}Y` } },
            { ageGroup: { contains: `${context.babyProfile.ageInMonths}M` } }
          ]
        }),
  ...
  private static calculateContextualScore(
    product: Product & { userActions: any[]; recommendations: any },
    context: RecommendationContext
