...
async function updateRecommendationScores(productId: string, feedback: number) {
  try {    await prisma.productRecommendation.upsert({
      where: { productId },
      update: {
...
function calculateProductScore(product: any, input: ProductBundlerInput): ProductScore {
  ...
  try {
    ...
    const products = await prisma.product.findMany({
      where: {
        ...
        OR: [
          { ageGroup: 'All Ages' },
          { ageGroup: { contains: `${Math.floor(input.ageInMonths / 12)}Y` } },
          { ageGroup: { contains: `${input.ageInMonths}M` } }
        ]
      }
    });

    // Score and rank products
    const scoredProducts: BundleProduct[] = products.map(product => {
      const { score, reasons } = calculateProductScore(product as BundleProduct, input);
      return {
        ...product,
        matchScore: score,
